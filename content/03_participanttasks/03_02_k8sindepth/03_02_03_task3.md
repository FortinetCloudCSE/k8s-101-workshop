...
after run `kubectl edit`, it will show you a VI EDITOR with opened yaml file, locate the text 
```
spec:
...
then change replicas from 1 to 10, then save and  exit the EDITOR. and check the new deployment with updated replicas. 
...
delete deployment after done the observation.
...
check the service use curlpod from cluster-internal
```bash
...
before restart 
```bash
ubectl get pod -o wide -l app=kubernetes-bootcamp
```
expected output 
```
...
```
then restart the deployment with rolling update. 

```bash
kubectl rollout restart deployment kubernetes-bootcamp
```

use `kubectl rollout status deployment kubernetes-bootcamp`  to check the restart status
