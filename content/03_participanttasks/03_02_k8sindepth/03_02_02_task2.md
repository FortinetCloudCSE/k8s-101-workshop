...
1. volumeMounts in container level
kubectl explain deployment.spec.template.spec.volumes
2. volumes in pod level
kubectl explain deployment.spec.tempalte.spec.containers.volumeMounts
3. find the /location of nginx
```
k exec -it po/nginx-pod-with-configmap-secret -- cat /etc/nginx/conf.d/default.conf  | grep "location /" -A 1
    location / {
