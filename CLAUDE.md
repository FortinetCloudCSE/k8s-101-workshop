...
- **The build is now 0 WARN, and `errorignore` in `scripts/repoConfig.json` is what keeps it there.** relearn's `layouts/partials/_relearn/menuPermalink.gotmpl` warns *unconditionally* whenever a `menu.shortcuts` entry has `.URL` and no `.PageRef` — it runs the value through `relLangURL` + `urls.Parse` and reports any result without a scheme/host. The `Workshop PDF` shortcut pointing at `k8s-101.pdf` trips this by construction, and it is not fixable by rewriting the URL: `pageRef` does not apply (the target is a file, not a page) and a fully-qualified URL is not reachable from `repoConfig.json`. **The fix was upstream:** CentralRepo PR #71 taught `hugo.jinja` to emit `site.Params.errorignore`, which `urlErrorReport.gotmpl` matches as a list of regexes against the offending URL. This repo now carries `"errorignore": ["^k8s-101\\.pdf$"]`. Verified 2026-08-19 on merged `main` (`4eeb17e`): 46 pages, 0 WARN.
  - This supersedes an earlier note here that said the WARN "cannot be fixed from this repo — don't try" and called it the documented baseline. That was true only until `errorignore` support existed upstream.
  - Consequence for future WARNs: **0 WARN is now the baseline, so any WARN is a regression.** Do not reach for `errorignore` first — it suppresses the report without checking the link, so a genuinely broken ref hidden behind an over-broad pattern will never be caught. Anchor every pattern (`^…$`) and add one only for a URL that is correct but unverifiable by Hugo, like this non-page file.
...
- **`pathtabs` / `pathtab` are already in the image — do NOT copy them from `ai-101`.** They were upstreamed to CentralRepo in PR #71 (merged 2026-08-19) and `ai-101` deleted its local copies, so there is nothing there to copy. This supersedes an earlier note here that pointed at `~/pythonProjects/ai-101/layouts/shortcodes/`. Following it now would create a repo-local `layouts/shortcodes/pathtabs.html`, and because `local_copy.sh` does a non-recursive `cp` of `layouts/shortcodes/*` over CentralRepo's, that file would **silently shadow** the shared one — upstream fixes would stop arriving with no warning and no build error. Just use the shortcodes; this repo needs no `layouts/` at all.
  - To turn a path on here, the only required step is `deploymentPaths` in `scripts/repoConfig.json` (`[{key, title}, …]`). The shared `pathtabs` calls `errorf` if that param is missing, and again if a block omits or duplicates a configured path — a content mistake fails the build rather than warning. See CentralRepo's `README.md` for the full parameter docs.
  - **Never rename a `deploymentPaths` title once readers have used it.** relearn keys each reader's stored selection on `anchorize(title)`, so a rename silently resets everyone to the first tab. Renaming a `key` is the safe, loud alternative — every `pathtab path=` must follow or the build fails.
...
**Add a second deployment path** (e.g. bring AKS back): read **Deployment Paths** above first. Add `deploymentPaths` to `scripts/repoConfig.json` — that is the only wiring needed, since the shortcodes come from the image; do **not** create `layouts/shortcodes/pathtabs.html` here. Then fill in `PATH_KEYS` / `PATH_TOKENS` in `scripts/lint_paths.py` (leave `PATH_TITLE_RE` disabled — it matches two ordinary tab titles here) and run the linter, expecting to add `ALLOWLIST` entries for conceptual prose that names both paths. Never hand-write `groupid="deploy-path"`.
